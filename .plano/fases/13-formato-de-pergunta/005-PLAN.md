...
must_haves:
  truths:
    - "O verificador reprova defeito injetado e aprova o repositório real na mesma execução"
    - "Os vinte pontos declarados existem, sem rótulo vazio e sem ponto não declarado"
    - "Os quatro runtimes instalam com os sete comandos e recebem o contrato"
    - "Nenhum executável do produto foi alterado pela fase, e o planejamento anterior ao ciclo continua respondendo"
  artifacts:
    - path: "up/bin/lib/perguntas.test.cjs"
      provides: "Verificador de mão dupla entre inventário e superfícies, com caso vermelho embutido"
    - path: ".plano/fases/13-formato-de-pergunta/PROVA.md"
      provides: "Relatório de prova da fase, com os limites da prova declarados"
  key_links:
    - from: "up/bin/lib/perguntas.test.cjs"
      to: "up/references/questioning.md"
      via: "leitura do inventário como fonte independente da expectativa"
    - from: "up/bin/lib/perguntas.test.cjs"
      to: "arquivos das superfícies declarados no inventário"
      via: "leitura das tags de pergunta e conferência dos rótulos obrigatórios"
---
...
<task id="1" type="auto">
<files>up/bin/lib/perguntas.test.cjs (novo)</files>
<contrato>
Dado um diretório raiz, o verificador devolve `{ ok, erros[] }` sem depender de nada além do
...
<action>
**Escrever o verificador do contrato de pergunta**

Escrever um módulo com a função `verificar(raiz)` que:
...
<verify><automated>node -e "const m=require('./up/bin/lib/perguntas.test.cjs');if(typeof m.verificar!=='function')process.exit(1)"</automated></verify>
<done>
`node up/bin/lib/perguntas.test.cjs` executa; a função `verificar` é exportada;
nenhum caminho é embutido além do inventário e da referência.

Prova registrada no resumo: execução do arquivo.
</done>
</task>

<task id="2" type="auto">
<files>up/bin/lib/perguntas.test.cjs</files>
<contrato>
O verificador roda o próprio caso vermelho a cada execução. Um verificador que só sabe passar
...
<action>
**Ver o verificador falhar contra defeito injetado e passar contra o real**

Implementar o `main()` em três etapas, com `assert`:
...
<verify><automated>node up/bin/lib/perguntas.test.cjs</automated></verify>
<done>
A execução única do arquivo exercita vermelho e verde; a etapa vermelha falha por três
tipos distintos; a etapa verde passa contra o repositório real e reporta 20 pontos verificados.

Prova registrada no resumo: saída do comando `node up/bin/lib/perguntas.test.cjs` colada no relatório de prova.
</done>
</task>

<task id="3" type="auto">
<files>.plano/fases/13-formato-de-pergunta/PROVA.md (novo)</files>
<contrato>
Num repositório onde o planejamento já responde, o brainstorm não pergunta o que está escrito.
...
<action>
**Smoke da regra de fato num repositório com planejamento populado**

Usar como tarefa de teste, fixa e declarada: **"acrescentar uma seção nova ao documento de
...
<verify><automated>grep -q "Regra de fato" .plano/fases/13-formato-de-pergunta/PROVA.md</automated></verify>
<done>
A tabela existe com no mínimo seis linhas; toda linha de fato cita arquivo e trecho;
nenhuma das seis candidatas foi emitida como pergunta.

Prova registrada no resumo: a própria seção do relatório, com as fontes citadas conferíveis por leitura.
</done>
</task>

<task id="4" type="auto">
<files>.plano/fases/13-formato-de-pergunta/PROVA.md</files>
<contrato>
As sete superfícies interativas apresentam pergunta com recomendação e motivo. A prova é o texto
...
<action>
**Smoke das sete superfícies**

Rodar o extrator abaixo e colar a saída na seção, agrupada por superfície:
...
<verify><automated>grep -q "Sete superfícies" .plano/fases/13-formato-de-pergunta/PROVA.md</automated></verify>
<done>
A seção lista as sete superfícies pelo nome, com a contagem de pontos de cada uma
somando 20; nenhum bloco com rótulo vazio.

Prova registrada no resumo: a saída do extrator no relatório.
</done>
</task>

<task id="5" type="auto">
<files>nenhum arquivo do repositório: instalação em diretório temporário do sistema, só leitura</files>
<contrato>
A instalação continua produzindo os sete comandos nos quatro runtimes, as quatro skills de
...
Instalar em diretório temporário, sem tocar na configuração real do dono:
...
<verify><automated>TMPUP=$(mktemp -d); CLAUDE_CONFIG_DIR=$TMPUP/claude GEMINI_CONFIG_DIR=$TMPUP/gemini OPENCODE_CONFIG_DIR=$TMPUP/opencode CODEX_HOME=$TMPUP/codex node up/bin/install.js --all --global >/dev/null && test "$(ls $TMPUP/claude/commands/up/*.md | wc -l)" = "7" && test "$(ls $TMPUP/gemini/commands/up/*.toml | wc -l)" = "7" && test "$(ls $TMPUP/opencode/command/up-*.md | wc -l)" = "7" && test "$(ls -d $TMPUP/codex/skills/up-*/SKILL.md | wc -l)" = "7" && test "$(ls -d $TMPUP/claude/skills/*/ | wc -l)" = "11" && for r in claude gemini opencode codex; do grep -q contrato_de_pergunta $TMPUP/$r/up/references/questioning.md || exit 1; done; rm -rf $TMPUP</automated></verify>
<done>
Todos os valores batem; as quatro skills de doutrina existem; nenhuma escrita fora do
diretório temporário.

Prova registrada no resumo: saída dos comandos no relatório de prova, seção "Regressão zero".
</done>
</task>

<task id="6" type="auto">
<files>nenhum arquivo do repositório: só leitura e execução de comando</files>
<contrato>
Projeto cujo diretório de planejamento foi escrito antes deste ciclo continua funcionando sem
migração. Este repositório é o caso de teste: o planejamento dele tem fases do ciclo 1 gravadas em convenção
antiga.
</contrato>
<action>
**Regressão do projeto com planejamento anterior ao ciclo**

Primeiro provar por diferença que nenhum executável mudou nesta fase:
...
<verify><automated>test -z "$(git diff --name-only $(git merge-base HEAD main)..HEAD -- up/bin | grep -v perguntas.test.cjs)" && for c in "init up" "roadmap analyze" "state-snapshot" "phase-plan-index 3"; do node up/bin/up-tools.cjs $c >/dev/null || exit 1; done</automated></verify>
<done>
Diferença vazia em `up/bin` fora do verificador; cinco comandos com `ok`; a observação
conhecida registrada como pré-existente com o requisito que a cobre.

Prova registrada no resumo: saída dos comandos no relatório de prova.
</done>
</task>

<task id="7" type="auto">
<files>.plano/fases/13-formato-de-pergunta/PROVA.md, up/bin/lib/perguntas.test.cjs</files>
<contrato>
A prova da fase é legível sem reexecutar nada, e diz o que foi provado, como, e o que ficou de
fora.

Arquivo hoje: `.plano/fases/13-formato-de-pergunta/PROVA.md`.
</contrato>
<action>
**Escrever o relatório de prova e commitar**

Consolidar as seções "Verificador (vermelho e verde)", "Regra de fato", "Sete superfícies",
...
<verify><automated>test -f .plano/fases/13-formato-de-pergunta/PROVA.md && node up/bin/lib/perguntas.test.cjs</automated></verify>
<done>
O relatório tem as cinco seções; a seção de limites existe e é específica; dois
commits atômicos.

Prova registrada no resumo: o arquivo e os dois hashes.
</done>
</task>
