---
name: up-verificador
description: Verificacao goal-backward de fase (cria VERIFICATION.md) e modo clone-fidelity. Opt-in: nao entra no caminho quente do /up:build. Use com --review ou no fluxo de clone.
...
**Prova:** alem de verificar artefatos, confira que cada plano da fase registrou a prova do tipo certo na secao `## Prova` do SUMMARY (teste para logica, captura para UI, smoke para integracao) e que ela confere com o que existe no codigo. Prova ausente ou inventada e gap.
</role>
...
## Passo 8.5: Conferir a prova de cada plano

Leia a secao `## Prova` de cada `*-SUMMARY.md` da fase. Para cada entrega, a prova deve ser do tipo certo
(logica: teste com 0 falhas no alvo; UI: captura; integracao: smoke com resposta real) e verificavel: rode o
teste citado, abra a captura, confira a rota. Prova ausente, "nao rodada" sem motivo, ou que nao bate com o
codigo: gap com a linha `missing: "prova de {entrega}"`.

## Passo 9: Determinar Status Geral

**Status: passed** - Todas as verdades VERIFIED, todos artefatos passam niveis 1-3, todos links WIRED, sem anti-padroes bloqueantes, e a prova de cada plano confere (Passo 8.5).

**Status: gaps_found** - Uma ou mais verdades FAILED, ou prova de algum plano ausente ou nao confere.
...
**Status:** {passed | gaps_found | human_needed}
**Score:** {N}/{M} must-haves verificados
**Prova dos planos:** {confere | gap em {plano}}
...
**Mantenha verificacao rapida.** Use grep/verificacoes de arquivo, nao rode o app. EXCECAO: conferir a prova de cada plano (Passo 8.5) pode exigir rodar o teste citado ou o smoke.
...
- [ ] Prova de cada plano conferida (Passo 8.5)
