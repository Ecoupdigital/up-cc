...
- **UP** (the live product) - A Portuguese-language meta-prompting system, published as `up-cc` on npm at **v3.0.0** (breaking-change major). Lives in `up/` (self-contained: `up/bin/`, `up/agents/`, `up/commands/`, `up/hooks/`, `up/workflows/`, `up/skills/`, `up/templates/`, `up/references/`). v2 collapsed the old fleet into **7 commands, 12 agents**, made it **brainstorm-first** (no creative work without exploring intent first) and **GitHub-native by default** (each phase runs in its own worktree/branch/issue and asks how to land it). v3 ("UP leve") removed the deterministic gate (`approvals.log`, `evidence=`), test seams and the anti-tautology heuristic, made verifier/reviewer opt-in, and merged the proof skills into **3 skills**. The model is capable; UP guides instead of policing.
